...
int main( int argc, char * argv[] ) {
  ...
  if (strstr(filename, ".txt") == NULL) {
    printf("The file entered is not a text (.txt) file.");
    exit(1);
  }

  FILE* input_open;
  input_open = fopen(filename, "r");
  ...
  char first_line[100];
  fgets(first_line, 100, input_open);
  ...
  FILE* output_open;
  output_open = fopen("output_file.txt", "w");
  ...
  for (int repeat_count = 0; repeat_count <= copyCount; repeat_count++) {
    for (int counter = 0; counter <= first_line; counter++) {
        ...
        fprintf(output_open, line_to_print);
    }
  }
